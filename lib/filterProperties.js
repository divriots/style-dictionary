...
import { reduce, assign, filter, isObject, isEmpty } from "lodash-es";
/**
 ...
 *   that matched the filter.
 */
function filterPropertyArray(properties, _filter) {
  // Go lodash!
  return filter(properties, _filter);
}
 ...
 */
function filterProperties(dictionary, _filter) {
  if (!_filter) {
    return dictionary;
  } else {
    return assign({}, dictionary, {
      allProperties: filterPropertyArray(dictionary.allProperties, _filter),
      properties: filterPropertyObject(dictionary.properties, _filter)
    });
  }
}
