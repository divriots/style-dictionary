...
`;
  ...
  to_ret.files = (config.files || []).map(function(file) {
    ...
    if (file.template) {
      ...
      throw new Error(
        "Please supply a format for file: " + JSON.stringify(file)
      );
    }
    return extend({}, file, ext);
  });
