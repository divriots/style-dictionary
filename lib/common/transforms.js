...
import { tinycolor as Color } from "@thebespokepixel/es-tinycolor";
import { camelCase, snakeCase, upperFirst, kebabCase } from "lodash-es";
const UNICODE_PATTERN = /&#x([^;]+);/g;

function isColor(prop) {
  return /^colors?$/.test(prop.attributes.category);
}

function isSize(prop) {
  return /^(sizes?)$/.test(prop.attributes.category);
}
...
function isNotFontSize(prop) {
  return !isFontSize(prop)
}

function isAsset(prop) {
  return /^assets?$/.test(prop.attributes.category);
}

function isContent(prop) {
  return /^contents?$/.test(prop.attributes.category);
}
...
export default {
  ...
  "attribute/cti": {
    ...
    transformer: function (prop) {
      ...
      for (let i = 0; i < prop.path.length && attrNames.length > 0; i++) {
        if (
          /^colors?/.test(generatedAttrs.category) &&
          /^modes?/.test(prop.path[i])
        ) {
          generatedAttrs['mode'] = prop.path[i + 1];
          prop.path.splice(i, 1);
        } else generatedAttrs[attrNames.pop()] = prop.path[i];
      }
  ...
  "name/cti/camel": {
    ...
    transformer: function(prop, options) {
      return camelCase([options.prefix].concat(prop.path).join(" "));
    }
  },
  ...
  "name/cti/snake": {
    type: "name",
    transformer: function(prop, options) {
      return snakeCase([options.prefix].concat(prop.path).join(" "));
    }
  },
  ...
  "name/cti/constant": {
    ...
    transformer: function(prop, options) {
      return snakeCase(
        [options.prefix].concat(prop.path).join(" ")
      ).toUpperCase();
    }
  },
  ...
  "name/cti/pascal": {
    ...
    transformer: function(prop, options) {
      return upperFirst(
        camelCase([options.prefix].concat(prop.path).join(" "))
      );
    }
  },
  ...
  "asset/path": {
    ...
    matcher: isAsset,
    transformer: function(prop) {
      return new URL(prop.value, location.href).href;
    }
  },
