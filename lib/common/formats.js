...
import { chain, template } from "lodash-es";
...
function iconsWithPrefix(prefix, properties, config) {
  return chain(properties)
    ...
    .map(function(prop) {
      ...
      var className =
        "." + config.prefix + "-icon." + prop.attributes.item + ":before ";
...
const withColorScheme = p=>p.attributes.category==='color' && !!p.attributes.mode

const colorsWithScheme = (dictionary)=>{
  return dictionary.allProperties.filter(withColorScheme).reduce((a,p)=>{
    const mode = p.attributes.mode;
    if(!a[mode]) a[mode]=[]
    a[mode].push(p);
    return a;
  },{})
...
export default {
  ...
  "css/variables": function(dictionary) {
    let styleSheet = `${fileHeader(this.options)}
:root{
${variablesWithPrefix("  --", dictionary.allProperties)}
}`
    for(const [scheme,colors] of Object.entries(colorsWithScheme(dictionary)))
      styleSheet +=`
@media (prefers-color-scheme: ${scheme}){
  :root{
${colors.map(c=> `    --${c.name.replace(`-${scheme}`,'')}: var(--${c.name});`).join('\n')}
  }
}`
    return styleSheet;
  },
   ...
   * ```
   */
  "scss/map-flat": template(templates.scss_map_flat),
   ...
   * ```
   */
  "scss/map-deep": template(templates.scss_map_deep),
  ...
  "scss/variables": function(dictionary) {
    let styleSheet = `${fileHeader(this.options, "short")}
${variablesWithPrefix("$", dictionary.allProperties, "short")}`
    for(const [scheme,colors] of Object.entries(colorsWithScheme(dictionary)))
    styleSheet +=`
@media (prefers-color-scheme: ${scheme}){
${colors.map(c=> `  $${c.name.replace(`-${scheme}`,'')}: $${c.name};`).join('\n')}
}`
    return styleSheet;
  },
  ...
  "scss/icons": function(dictionary, config) {
    return (
      ...
      iconsWithPrefix("$", dictionary.allProperties, config, "short")
    );
  },
  ...
  /**
   * Creates an array of all dictionary properties
   *
   * @memberof Formats
   * @kind member
   * @example
   ...
   * ```
   */
  "javascript/properties": ({ allProperties }) =>
    `export default ${JSON.stringify(allProperties, null, " ")}`,
   ...
   * ```
   */
  "android/colors": template(templates.android_colors),
   ...
   * ```
   */
  "android/dimens": template(templates.android_dimens),
   ...
   * ```
   */
  "android/fontDimens": template(templates.android_fontDimens),
   ...
   * ```
   */
  "android/integers": template(templates.android_integers),
   ...
   * ```
   */
  "android/strings": template(templates.android_strings),
   ...
   * ```
   */
  "ios/macros": template(templates.ios_macros),
   ...
   */
  "ios/plist": template(templates.ios_plist),
   ...
   */
  "ios/singleton.m": template(templates.ios_singleton_m),
   ...
   * @todo Add example and usage
   */
  "ios/singleton.h": template(templates.ios_singleton_h),
   ...
   * @todo Add example and usage
   */
  "ios/static.h": template(templates.ios_static_h),
   ...
   * @todo Add example and usage
   */
  "ios/static.m": template(templates.ios_static_m),
   ...
   * @todo Add example and usage
   */
  "ios/colors.h": template(templates.ios_colors_h),
   ...
   * @todo Add example and usage
   */
  "ios/colors.m": template(templates.ios_colors_m),
   ...
   * @todo Add example and usage
   */
  "ios/strings.h": template(templates.ios_strings_h),
   ...
   * @todo Add example and usage
   */
  "ios/strings.m": template(templates.ios_strings_m),
   ...
   * @todo Add example and usage
   */
  "ios-swift/class.swift": template(templates.ios_swift_class_swift),
   ...
   * @todo Add example and usage
   */
  "ios-swift/enum.swift": template(templates.ios_swift_enum_swift),
  // Css templates
   ...
   * @todo Add example and usage
   */
  "css/fonts.css": template(templates.css_fonts_css),
  ...
  "sketch/palette": function(dictionary) {
    ...
    to_ret.colors = chain(dictionary.allProperties)
      .filter(function(prop) {
        return (
   ...
   * ```
   */
  'flutter/class.dart': template(templates.flutter_class_template)
};
