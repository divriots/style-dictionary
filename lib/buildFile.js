...
function buildFile(destination, format, platform, dictionary, filter) {
  ...
  var PROPERTY_NAME_COLLISION_WARNINGS =
  ...
  Object.keys(nameCollisionObj).forEach(propertyName => {
    if (nameCollisionObj[propertyName].length > 1) {
      let collisions = nameCollisionObj[propertyName]
        .map(properties => {
          let propertyPathText = properties.path.join(".");

          let valueText = properties.value;
          return propertyPathText + "   " + valueText;
        })
        .join("\n        ");
      GroupMessages.add(
        PROPERTY_NAME_COLLISION_WARNINGS,
        `Output name ${propertyName} was generated by:\n        ${collisions}`
      );
    }
  });

  let propertyNamesCollisionCount = GroupMessages.count(
    PROPERTY_NAME_COLLISION_WARNINGS
  );
  if (propertyNamesCollisionCount > 0) {
    ...
    let title = `While building ${destination}, token collisions were found; output may be unexpected.`;
    let help = [
  ...
  return { [fullDestination]: format(filteredProperties, platform) };
}
